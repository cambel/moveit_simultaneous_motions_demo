...
int main(int argc, char** argv) {
  ...
  moveit::planning_interface::MoveGroupInterface dual_arm_group(
      "rocket_and_groot");
  ...
  // w x y z
  Eigen::Quaternionf rocket_q =
      Eigen::Quaternionf(0.0044319521005895665, -0.0018064082028716572,
                         0.714190127940822, -0.6999353940485185);
  Eigen::Quaternionf groot_q =
      Eigen::Quaternionf(0.7171097271676862, -0.6959453209354478,
                         -0.029260144371181365, -0.02361341612136324);
  ...
  for (int i; i < 100; i++) {
    ...
    groot_q = Eigen::AngleAxisf(x_rotation, Eigen::Vector3f::UnitX()) *
              Eigen::AngleAxisf(y_rotation, Eigen::Vector3f::UnitY()) *
              Eigen::AngleAxisf(z_rotation, Eigen::Vector3f::UnitZ()) * groot_q;
